...
| [PlayerDataResolverTests](Tests/Editor/PlayerDataResolverTests.cs) | `PlayerDataResolver` player-layout selection (loose `globalgamemanagers` vs `data.unity3d`), bundle entry lookup, and header version read, driven against a synthesized UnityFS bundle. Offline, no fixtures. | ✅ |
...
| [ImportProjectSettingsTests](Tests/Editor/ImportProjectSettingsTests.cs) | Real `ImportProjectSettings.ExportProjectSettings` over per-version `globalgamemanagers` fixtures + `classdata.tpk`: tpk version-resolution + YAML export on every matrix editor; core-setting correctness on the editor matching a fixture. Fixtures are generated by [generate-project-settings-fixtures](.github/workflows/generate-project-settings-fixtures.yml). Offline, runs in CI. | 🟡 |
...
- 🟡 `ImportProjectSettings.ExportProjectSettings` — [ImportProjectSettings.cs](Editor/Core/Config/Common/ImportProjectSettings.cs). Covered by [ImportProjectSettingsTests](Tests/Editor/ImportProjectSettingsTests.cs). Fixtures are produced by the [generate-project-settings-fixtures](.github/workflows/generate-project-settings-fixtures.yml) workflow (one `globalgamemanagers` per matrix Unity version, built by that exact editor) and committed under `Tests/Editor/Fixtures/GlobalGameManagers/<version>/`. The test auto-discovers them; on the editor matching a fixture it asserts the core stable settings export, on every other editor it asserts resolution + no-crash. **To populate / refresh:** run the workflow from the Actions tab — no manual project or build work. **Gap:** the matching-editor assertion checks a small universal core set (`AudioManager`, `TimeManager`, `TagManager`, `DynamicsManager`, `QualitySettings`), not every version-specific setting; pin a fixture's own `expectedSettings` in its `fixture.json` to assert more. PlayerSettings/GraphicsSettings are excluded by design (layout shifts across patch releases).
